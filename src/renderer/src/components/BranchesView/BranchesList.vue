  ...
  <a-card class="branches-table-card" :loading="loading">
    <template #title>
      <div class="card-header">
        <span>分支列表</span>
        <a-input v-model:value="searchQuery" placeholder="搜索分支" class="search-input" allow-clear>
          <template #prefix>
            <SearchOutlined />
          </template>
        </a-input>
    ...
    <a-table :data-source="filteredBranches" :columns="columns" :pagination="false" :scroll="{ x: 'max-content' }">
...
import { Card, Input, Table, Tag, ButtonGroup, Button, Descriptions, DescriptionsItem } from 'ant-design-vue';
...
const columns = ref([
  ...
  },
  {
    ...
    key: 'commitsCount',
    width: 100,
    sorter: (a, b) => a.commitsCount - b.commitsCount,
  },
  {
    title: '最近更新',
    key: 'lastUpdate',
