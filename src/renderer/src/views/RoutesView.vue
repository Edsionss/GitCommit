    ...
    >
      <template #expandedRowRender="{ record }">
        <a-descriptions bordered size="small">
          ...
          <a-descriptions-item label="菜单名称 (Title)">{{ record.meta.title }}</a-descriptions-item>
          <a-descriptions-item label="路由路径 (Path)">{{ record.path }}</a-descriptions-item>
          <a-descriptions-item label="组件路径 (Component)">{{ record.componentPath }}</a-descriptions-item>
          ...
          <a-descriptions-item label="缓存页面 (keepAlive)">
            <a-tag :color="record.meta.keepAlive ? 'green' : 'red'">{{ record.meta.keepAlive }}</a-tag>
          </a-descriptions-item>
        </a-descriptions>
      </template>
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'isMenu'">
          <a-switch :checked="record.isMenu" @change="(checked) => onFieldChange(record, 'isMenu', checked)" />
        </template>
        <template v-if="column.key === 'keepAlive'">
          <a-switch :checked="record.meta.keepAlive" @change="(checked) => onFieldChange(record, 'keepAlive', checked)" />
        </template>
