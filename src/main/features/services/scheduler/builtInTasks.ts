...
export const builtInTasks: BuiltInTask[] = [
  {
    ...
    execute: async () => {
      console.info('Executing built-in task: fetch-stock-news')
      try {
        const result = await stockNewsService.scrapeAndInsertNews()
        console.info('Built-in task "fetch-stock-news" completed successfully.')
        return result
      } catch (error) {
        console.error('Error executing built-in task "fetch-stock-news":', error)
        throw error
      }
    }
  },
  {
    ...
    execute: async () => {
      console.info('Executing built-in task: scheduled-shutdown')
      try {
        const result = await systemToolsService.shutdownImmediately()
        console.info('Built-in task "scheduled-shutdown" executed.')
        return result
      } catch (error) {
        console.error('Error executing built-in task "scheduled-shutdown":', error)
